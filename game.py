...
# --- Initialization ---
pygame.init()
pygame.mixer.init()  # Initialize the mixer module for music
...
DUNGEON_MUSIC = None  # Dungeon music file path
BOSS_MUSIC = None     # Boss music file path
DRAGON_MUSIC = None   # Dragon music file path
...
def load_music(filename):
    ...
    try:
        ...
        print(f"Unable to load music: {filename}. Error: {e}")
        return None

# --- Music Functions ---
def play_music(music_file, loop=-1):
    """Play a music file if music is enabled."""
    ...
    if music_file != CURRENT_MUSIC:
        try:
            ...
            print(f"Unable to play music: {music_file}. Error: {e}")
...
class LockedDoor(GameObject):
    def __init__(self, x, y):
        super().__init__(x, y, 'door', blocks=True)
        self.locked = True

    def unlock(self, message_log):
        if self.locked:
            self.locked = False
            self.blocks = False
            self.sprite = None  # Make it invisible when unlocked
            message_log.add_message('The door unlocks!', GREEN)
...
class Key(Item):
    def use(self, inventory, message_log, objects=None, target_x=None, target_y=None, **kwargs):
        if target_x is None or target_y is None:
            message_log.add_message('Select a door to unlock.', YELLOW)
            return 'targeting'
        
        # Check if there's a locked door at the target location
        door = next((obj for obj in objects if isinstance(obj, LockedDoor) and
                     obj.x == target_x and obj.y == target_y), None)
        
        if door:
            door.unlock(message_log)
            # Remove the key from inventory
            inventory.remove_item(self.owner)
            return 'used'
        else:
            message_log.add_message('There is no door to unlock there.', RED)
            return 'cancelled'
...
def place_objects(spawns, objects, level):
    ...
    for i_x, i_y in spawns['i']:
        if random.random() < 0.4: # Chance to spawn an item
            ...
            objects.append(item)
    for d_x, d_y in spawns['D']: objects.append(LockedDoor(d_x, d_y))
    for k_x, k_y in spawns['K']: objects.append(GameObject(k_x, k_y, 'key', item=Key()))
...
def calculate_exploration_percentage(game_map, total_floor_tiles):
    explored_tiles = sum(1 for x in range(MAP_WIDTH) for y in range(MAP_HEIGHT)
                         if not game_map[x][y].blocked and game_map[x][y].explored)
    return explored_tiles / total_floor_tiles if total_floor_tiles > 0 else 0
...
def draw_all(surface, game_map, objects, player, fov_map, light_map, message_log, game_state, dungeon_level, targeting_mode, target_pos):
    ...
    if game_state == 'HELP_MENU':
        ...
        # Add restart option for GAME_OVER and VICTORY states
        if game_state in ['GAME_OVER', 'VICTORY']:
            restart_text = font.render("Press R to Restart or ESC to Quit", True, WHITE)
            surface.blit(restart_text, restart_text.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 50)))
...
def draw_main_menu(surface):
    """Draws the main menu on the pre-scaled background."""
    if SPRITES.get('menu_background'):
        surface.blit(SPRITES['menu_background'], (0, 0))
    else:
        # Fallback to a black background if the image failed to load
        surface.fill(BLACK)
    
    # Draw menu options
    start_text = font.render("Press ENTER to Start", True, WHITE)
    surface.blit(start_text, start_text.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 50)))
    
    quit_text = font.render("Press ESC to Quit", True, WHITE)
    surface.blit(quit_text, quit_text.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2 + 100)))
    ...
    for instruction in instructions:
        instruction_text = help_font.render(instruction, True, GRAY)
        surface.blit(instruction_text, instruction_text.get_rect(center=(SCREEN_WIDTH/2, y_offset)))
        y_offset += 25
        ...
        if event.type == pygame.KEYDOWN:
            if game_state == 'HELP_MENU':
                ...
                if event.key in (pygame.K_UP, pygame.K_k): game_state = player_move_or_attack(0, -1, player, objects, game_map, message_log, dungeon_level, game_state_main)
                    ...
                    is_enabled = toggle_music()
                    message_log.add_message(f"Music {'enabled' if is_enabled else 'disabled'}", YELLOW)
...
def player_move_or_attack(dx, dy, player, objects, game_map, message_log, dungeon_level, game_state):
    ...
    door = next((obj for obj in objects if isinstance(obj, LockedDoor) and obj.x == x and obj.y == y), None)
    if door and door.locked:
        # Check if player has key
        has_key = any(item.name == 'key' for item in player.inventory.items)
        if has_key:
            message_log.add_message('Use the key to unlock the door.', YELLOW)
        else:
            message_log.add_message('The door is locked. You need a key to open it.', RED)
        return game_state
        ...
        if obj.fighter and obj.fighter.is_boss:
            ...
            if dungeon_level == 5 and obj.name == 'dragon':
                ...
                if princess:
                    ...
                    # Stop all music after defeating the dragon
                    if MUSIC_ENABLED:
                        stop_music()
                    # Set the dragon_defeated flag
                ...
                # Switch back to dungeon music after defeating a boss
                if DUNGEON_MUSIC and MUSIC_ENABLED:
                    play_music(DUNGEON_MUSIC)
...
def get_current_boss(objects):
    """Get the current boss if one exists."""
    return next((obj for obj in objects if obj.fighter and obj.fighter.is_boss), None)

def main():
    global stairs

    try:
        # Use the correct filename ('menu.jpg' or 'menu.png')
        menu_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'assets/menu.png')
        menu_img = pygame.image.load(menu_path).convert()
        # Scale it ONCE to the screen size for a perfect fit
        SPRITES['menu_background'] = pygame.transform.smoothscale(menu_img, (SCREEN_WIDTH, SCREEN_HEIGHT))
    except pygame.error as e:
        print(f"Unable to load menu background image: {e}")
        SPRITES['menu_background'] = None
    ...
    load_and_scale_sprite('princess', 'assets/princess.png', 0.65)
    load_and_scale_sprite('key', 'assets/key.png', 0.5)
    load_and_scale_sprite('door', 'assets/door.png', 1.0)


    # Load music files
    global DUNGEON_MUSIC, BOSS_MUSIC, DRAGON_MUSIC
    DUNGEON_MUSIC = load_music('assets/dungeon.mp3')
    ...
    # Initialize boss encounter tracking
    boss_encountered = False
    
    # Play initial dungeon music
    if DUNGEON_MUSIC and MUSIC_ENABLED:
        play_music(DUNGEON_MUSIC)
    
    # Flag to track if the dragon has been defeated
    ...
    while running:
        ...
        # Check exploration percentage and reveal key if requirement is met
        exploration_percentage = calculate_exploration_percentage(game_map, total_floor_tiles)
        if exploration_percentage >= EXPLORATION_REQUIREMENT:
            # Reveal the key
            for obj in objects:
                if obj.name == 'key' and obj.sprite is None:
                    obj.sprite = SPRITES.get('key')
                    message_log.add_message('You found a key!', GOLD)
        
        # Check if a boss is present and switch music accordingly
        # Only do this if the dragon hasn't been defeated
        if not dragon_defeated:
            current_boss = get_current_boss(objects)
            # Only play boss music if the boss is in the player's field of view and not yet encountered
            if current_boss and fov_map[current_boss.x][current_boss.y] and not boss_encountered:
                boss_encountered = True
                # Determine which music to play based on the boss type
                if dungeon_level == 5 and current_boss.name == 'dragon':
                    if DRAGON_MUSIC and MUSIC_ENABLED and CURRENT_MUSIC != DRAGON_MUSIC:
                        play_music(DRAGON_MUSIC)
                else:
                    if BOSS_MUSIC and MUSIC_ENABLED and CURRENT_MUSIC != BOSS_MUSIC:
                        play_music(BOSS_MUSIC)
            elif not current_boss and boss_encountered:
                # Boss has been defeated, reset the encounter flag and play dungeon music
                boss_encountered = False
                if DUNGEON_MUSIC and MUSIC_ENABLED and CURRENT_MUSIC != DUNGEON_MUSIC:
                    play_music(DUNGEON_MUSIC)
            elif DUNGEON_MUSIC and MUSIC_ENABLED and CURRENT_MUSIC not in [DUNGEON_MUSIC, None] and not current_boss:
                # If no boss is present, play dungeon music
                play_music(DUNGEON_MUSIC)
        ...
        if game_state == 'LEVEL_UP':
            ...
            if dungeon_level > MAX_LEVELS: game_state = 'VICTORY'
                ...
                game_map, objects, total_floor_tiles = make_map(player, dungeon_level)
                # Reset boss encounter flag for the new level
                boss_encountered = False
                # Reset dragon defeated flag for the new level (only relevant for level 5)
                dragon_defeated = False
